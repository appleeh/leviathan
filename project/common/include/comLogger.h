...
public:
	...
	E_LOG_INIT_RES initLog(STLogInfo *pLogInfo);
	void OpenLogger();
...
private:
	...
	STFileList *m_pLogFileListForDelete;
	CQueueS	m_sLogQueue;
	fp_DataProc m_fpDataProc;
	THREAD_COND *m_pCondition;
	void *	m_pObj;	// 함수포인터에 넘길 객체 등록

	bool _initCom(TCHAR *pDir, TCHAR *pName, TCHAR *pFileExtension = g_szLogExtension, TCHAR *pKeepDuration = g_szKeepDuratrion);
...
extern bool	detachFullPath(const TCHAR *fullpath, TCHAR *dir, TCHAR *name, TCHAR *Extension);
